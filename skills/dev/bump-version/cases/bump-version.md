# Test: dev:bump-version

## Case: happy-path-with-versions
stub: with-openspec
semantic:
  - script_invoked: skill calls `bump-namespace.sh dev` from `${CLAUDE_SKILL_DIR}/scripts/`
  - dependency_resolution: output indicates dependency check across `.versions` / `.manifest` for dev namespace
