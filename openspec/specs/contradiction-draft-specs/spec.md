...
Draft-spec loading in `contradiction.py`: enables contradiction scanning for capabilities that exist only in a change's `specs/` directory (not yet in `index.yaml`).
## Requirements
