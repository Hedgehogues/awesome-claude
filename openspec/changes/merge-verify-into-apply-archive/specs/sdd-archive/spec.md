...
### Requirement: Standalone spec-verify command

**Reason:** Spec-verify логика инлайнится в `/sdd:archive` как обязательный шаг после merge specs.

**Migration:** Перестать вызывать `/sdd:spec-verify` напрямую — verify запускается автоматически в `/sdd:archive`. Для проверки live spec'а без архивации — заводи новый change через `/sdd:propose`.
