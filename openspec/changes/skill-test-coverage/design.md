...
### D17: Skills reorganization — каждый скилл в отдельной папке

Текущая структура: `skills/sdd/propose.md`, `skills/sdd/apply.md`, etc. Скрипты лежат в `skills/sdd/scripts/` — это не масштабируемо при добавлении скриптов и cases.

**Решение:**
```
skills/sdd/propose/
├── skill.md         # основной скилл
├── scripts/         # скрипты для скилла
│   └── validate.py
└── cases/           # тест-кейсы (опционально)
...
Это касается всех неймспейсов: `dev`, `sdd`, `report`, `research`. Каждый скилл получает:
- Папка `<namespace>/<skill>/`
- Файл `skill.md` (вместо `<skill>.md`)
- Поддиректория `scripts/` (внутри папки скилла)
- Поддиректория `cases/` (если есть)

Зеркалирование:同样 структура в `.claude/skills/`.

Обоснование: масштабируемость, изоляция скилла (весь артефакт в одной папке), лучше видна иерархия.
