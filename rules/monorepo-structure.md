...
```
packages/
├── <service-a>/   # e.g. API server (own Makefile, pyproject.toml, tests/)
├── <service-b>/   # e.g. CLI tool (own Makefile, pyproject.toml, tests/)
└── <service-c>/   # e.g. Frontend SPA (own Makefile, package.json, e2e/)
```
...
3. **Root Makefile includes all test scopes**:
   ...
   - `make e2e` — E2E tests (requires running Docker)
